...
# Tunnel Server Daemon Uninstallation Script
# Run with: sudo ./uninstall-server-daemon.sh
...
confirm_uninstall() {
    echo
    log_warning "This will completely remove the tunnel server daemon from your system."
    log_warning "All configuration files, certificates, and logs will be deleted."
    echo
...
remove_config() {
    log_info "Removing configuration and certificates..."
    
    if [[ -d "$CONFIG_DIR" ]]; then
        echo
        read -p "Remove configuration directory $CONFIG_DIR (including certificates)? (y/N): " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            rm -rf "$CONFIG_DIR"
...
main() {
    log_info "Starting tunnel server daemon uninstallation..."
    ...
    echo
    log_info "The tunnel server daemon has been completely removed from your system."
}
